# 🛣️️ Road-map

> [👈 Back to README](./README.md)

## 🔴 P1

- [ ] Add keyboard drawer support ➕ 2026-05-30
  - [x] Add keyboard drawer config ➕ 2026-05-30 ✅ 2026-05-30
  - [x] Add `just` recipes to build the keyboard drawer. ➕ 2026-05-30 ✅ 2026-05-30
  - [x] Add `kg-layout.yaml` to `.gitignore` ➕ 2026-05-30 ✅ 2026-05-30
  - [x] Update the `README.md` to use the `glove80-draw.png`. ➕ 2026-05-30 ✅ 2026-05-30
  - [ ] Add a GitHub workflow to use the keyboard drawer. ➕ 2026-05-11

## 🟠 P2

- [ ] Docs ➕ 2026-05-30
  - [x] Update the LICENSE ➕ 2026-05-30 ✅ 2026-05-30
- [ ] Add links to the Sunaku work. ➕ 2026-05-11
  - [ ] [Taming home row mods with bilateral combinations - The Terminal Programmer](https://sunaku.github.io/home-row-mods.html#problem) ➕ 2026-05-11
  - [ ] https://github.com/sunaku/glove80-keymaps ➕ 2026-05-11
  - [ ] [peccu/zmk-cheat-sheet: ZMK cheat sheet picked up from official document.](https://github.com/peccu/zmk-cheat-sheet) ➕ 2026-05-11

## 🟡 P3

- [ ] Learn how to use the RGB stuff. ➕ 2026-05-11
- [Repository search results](https://github.com/search?q=glove-80&type=repositories)
- [glove80-config/packages/draw.nix at main · MattSturgeon/glove80-config](https://github.com/MattSturgeon/glove80-config/blob/main/packages/draw.nix)
- [glove80-layout/justfile at main · frozolotl/glove80-layout](https://github.com/frozolotl/glove80-layout/blob/main/justfile)
- [keymap drawer - Google Search](https://www.google.com/search?q=keymap drawer)
- [keymap-drawer/CONFIGURATION.md at main · caksoylar/keymap-drawer](https://github.com/caksoylar/keymap-drawer/blob/main/CONFIGURATION.md)
- [keymap-drawer/.github/workflows/draw-zmk.yml at main · caksoylar/keymap-drawer](https://github.com/caksoylar/keymap-drawer/blob/main/.github/workflows/draw-zmk.yml)
- [Keymap Drawer · Streamlit](https://keymap-drawer.streamlit.app/?example_yaml=3x5.yaml)
- [keymap drawer themes - Google Search](https://www.google.com/search?q=keymap drawer themes)
- [zmk-config/docs/export/brain.svg at master · Wesztman/zmk-config](https://github.com/Wesztman/zmk-config/blob/master/docs/export/brain.svg)
- [keymap-drawer/CONFIGURATION.md at main · caksoylar/keymap-drawer](https://github.com/caksoylar/keymap-drawer/blob/main/CONFIGURATION.md)
- [NikolaM-Dev/base-repo_tmpl: Base Repository Template 🖨️🪴](https://github.com/NikolaM-Dev/base-repo_tmpl)
- [alt and ctrl equivalents to mac - Google Search](https://www.google.com/search?q=alt and ctrl equivalents to mac)

---

## 🗃️ Archive

- [x] Fix the Windows layer problem. Currently it is not able to use the other layers ➕ 2026-05-30 ✅ 2026-05-30
- [x] Improve the layouts adding the custom behavior. ➕ 2026-05-11 ✅ 2026-05-30
  - [x] Add copy and paste support ✅ 2026-05-11 ➕ 2026-05-11
  - [x] Add support for smart caps words ✅ 2026-05-11 ➕ 2026-05-11
- [x] Add windows layer to avoid conflicts with home row mods ➕ 2026-05-30 ✅ 2026-05-30
- [x] Add key repetition. ✅ 2026-05-11
- [x] Improve the Magic layer. ✅ 2026-05-11
- [x] Add base important layouts. ✅ 2026-05-11
  - [x] Symbols ✅ 2026-05-10
  - [x] Functions ✅ 2026-05-10
  - [x] Numbers ✅ 2026-05-10
  - [x] Cursor ✅ 2026-05-10
  - [x] Mouse ✅ 2026-05-11
- [x] Improve the `thumb` behavior ✅ 2026-05-11
- [x] Improve the home rows feeling using custom tapping terms ✅ 2026-05-10
- [x] Add Home rows ✅ 2026-05-10
